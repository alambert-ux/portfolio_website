...
*"Someone's sitting in the shade today because someone planted a tree a long time ago."*  
  ...
  `- Buffett, Warren | iconic American investor, business tycoon and philanthropist``
...
Seeking to build a maximally profitable property portfolio and evaluate whether Crossrail promises further profit opportunities for nearby property-holders, I provide a comprehensive methodology for the specification, optimization and application of an ensemble model designed to predict London house prices. 

# The Data

Throughout these analyses, two key datasets are utilised: the first, an in-sample dataset containing 36 variables representing house price characteristics, as well as the final sale price of the property; and the second, an out-of-sample dataset containing 36 (non-identical) variables, including the property's asking price - but not its final sale price. 
...
Using the in-sample dataset including the actual sale prices and characteristics of 13,998 properties, I train and stack five predictive models, tuning both features and parameters to minimize Root Mean Square Error (RMSE) and build a final ensemble. 
...
*Note: because of the computational intensity of some models, I knit my .Rmd using a pre-loaded environment, rather than re-running all models.*
...
I now prepare the clean data to be suitable for modelling and out-of-sample (oos) prediction. 
...
**Estimating a correlation table between prices and other continuous variables**
...
The majority of these relationships are self-explanatory: we expect current and potential co2 emissions to be related positively; we expect co2 emissions and house size (number of rooms, total floor area) to be related positively; and we expect price and house size to be related positively. Further, since London zones increase in number as distance from the city center increases, we naturally expect London zone and price to be negatively correlated, and London zone and number of tube lines to be negatively correlated. It is also intuitive that the more tube lines there are, the fewer rail lines, and vice versa. This too is a likely product of being closer to or further from the city center - the closer to the center, the more tube lines and the fewer rail lines. Latitude and longitude relationships are more interesting and unexpected: lower average income is associated with higher longitude, meaning that the more East a property, the lower the average income of its buyer, and since latitude is negatively correlated with number of rail lines, the more north a property, the fewer rail lines present. 

# Training Prediction Algorithms

In order to run prediction algorithms, I take the following approach, focusing first on Features, and then on Model Tuning:
...
I measure the quality of my predictions via Root Mean Square Error and R-Squared (R2) on the testing data set. The model which minimises RMSE and maximises R2 on the testing data is the highest performing model, which makes the highest quality predictions (with the least error).
...
I decide to proceed with all variables in the final_equation specification for my subsequent models, and take the following steps in order to optimise them on the basis of selected features parameter tuning:
...
I first try a generic GBM using recommended parameters.. 
