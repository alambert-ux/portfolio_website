...
description: "AirBnB Analytics | A Deep-Dive into Copenhagen's Holiday Rental Market"
draft: false
image: pic06.jpg 
keywords: ""
slug: hillary
...
*"No one remembers who climbed Mount Everest the second time."*  
  ...
  ``- Hillary, Edmund | legendary New Zealand mountaineer, explorer and philanthropist, and leader of the first expedition to summit Mount Everest``

# **Airbnb Analytics** 

## **A Deep-Dive into Copenhagen's Holiday Rental Market**

**Note:** *this was a group project conducted in a team of three. My responsibility was to produce all visualisations and analyses related to model specification and selection, and to project manage timely, high-quality output. I also proofed and cleaned all the code and authored our final analyses and reflections.*

---

# Executive Summary

See [this Presentation Deck](https://drive.google.com/file/d/1u1gsDwQgdQeGZ8KQDl14y-wV1-I7IK2M/view?usp=sharing "A Deep-Dive into Copenhagen's Holiday Rental Market"){target="_blank"} for a full project report, including a summary of the research process, key results and a brief project evaluation. 
...
What is the best model for prediction of the overall price of AirBnB accommodation for 2 people wishing to spend 4 nights in Copenhagen?

### Methodology

This project is initially composed of Exploratory Data Analysis and Geospatial Analysis, which offers more context on variables of interest and their spatial, as well as statistical, distributions. Subsequently, we build, evaluate and select an optimal regression model to predict the price of an AirBnB stay in Copenhagen.
...
This chapter focuses on describing the process behind building our optimal model, guiding the reader through 7 preliminary models before identifying, specifying, testing and comparing linear regression model performance. We conclude by identifying a model which we find explains the greatest proportion of variance in the price of AirBnB listings while upholding core linear regression assumptions and showing the least overfitting of all tested models. 

### The Best Model

In summary, our "best", or optimal, model has the following characteristics:

- Predictors across 4 key categories: Facility & Location, Host & Reviews, Flexibility and Amenities
- The least overfitting of all models tested
- No problematic collinearity 
- A point estimate for the price of a 4-night stay in Copenhagen for 2 people of $1949 with a confidence interval of $1309 to $2597
...
First, we load in the Copenhagen AirBnB data, which has been scraped from the AirBnB website using vroom. We also clean the variable names as we import the data, to allow us to push on with our EDA without extensive renaming. 

```{r loading_data_copenhagen, messages=FALSE, warnings=FALSE, results='hide'}

#we use vroom to load the data directly from the AirBnB Website and it is faster than reads::read_csv
listings <- vroom("http://data.insideairbnb.com/denmark/hovedstaden/copenhagen/2020-06-26/data/listings.csv.gz") %>% 
  clean_names() #we change space to _ and convert upper case letters to lower case ones
...
```{r EDA_raw_glimpse, results='hide'}
...
```{r EDA_summary_stats_skim, results='hide'}
...
```{r recoding_variables, echo=FALSE, results='hide', messages=FALSE, warnings=FALSE}

listings_cleaned <- listings %>% 
  
  mutate(
    #we recode some variables to factors 
   ...
   #we recode some variables to numeric 
    host_response_rate = parse_number(host_response_rate), #we use parse_number as some of the variables have dollar signs or % signs, and thus as.numeric would create all N/A's for these values
...
#we now verify the conversion of data types for the above parsed variables 
...
#we find that in all cases the conversion has been successful, since they are all now of type double
```

We now inspect the new dataset:
```{r, skim_data_cleaned, results='hide'}
...
```{r, checking_listings_cleaned, results='hide'}
...
```{r, listing_excluded, results='hide'}
listings %>% 
...
```{r identifying_missing_values, results='hide'}
...
- what does this missingness indicate, and therefore how should we address it?

*In addition, we employ the chunk below to investigate the questions:*

- How many variables have missing values for cleaning_fee?

We find that the number of missing values for cleaning_fee is 8968. 

- What is the most likely reason for the missing observations of cleaning_fee? In other words, what does a missing value of cleaning_fee indicate?
...
```{r factor_prop_type, results='hide'}
...
```{r factor_percentage_datapoints, results='hide'}
...
```{r factor_prop_type_simplified, results='hide'}
...
```{r factor_acommodates, results='hide'}
...
```{r accommodates_simplified, results='hide'}
...
```{r factor_convert_datatype, results='hide'}
...
``` {r data_filtering_business_travel_ready, results='hide'}
...
``` {r data_filtering_min_nights, results='hide'}
...
``` {r data_filtering_cleaning_fee, results='hide'}
...
```{r, filter_minimum_nights, results='hide'}
...
```{r check_data_types, results='hide'}
...
```
Though we have already performed a relatively extensive comparison of models 1-7, there are still many variables we haven't explored. In the next section, we included a greater number of explanatory variables and constructed new variables to form the best model for predictions.
...
### Our Optimised Variable Categories
